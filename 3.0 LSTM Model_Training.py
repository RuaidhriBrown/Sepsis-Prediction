...
lstm_model.add(LSTM(64, input_shape=(X_train_reshaped.shape[1], X_train_reshaped.shape[2]), return_sequences=True))
...
lstm_history = lstm_model.fit(X_train_reshaped, y_train, epochs=50, batch_size=64, validation_split=0.2, callbacks=[early_stopping])

# Save the LSTM model
lstm_model.save('lstm_model.keras')

# Evaluate the LSTM model
y_pred_prob_lstm = lstm_model.predict(X_test_reshaped).ravel()

# Adjust threshold to reduce false negatives
threshold = 0.3  # Lowered threshold for higher sensitivity
y_pred_lstm = (y_pred_prob_lstm > threshold).astype(int)
...
print(f"LSTM Model ROC AUC Score: {roc_auc_lstm:.2f}")
...
y_pred_ensemble_prob = (y_pred_prob_lstm + y_pred_rf_prob) / 2
...
axs[0, 0].set_title('Confusion Matrix - LSTM + Random Forest Ensemble Model')
...
axs[0, 1].set_title('Normalized Confusion Matrix - LSTM + Random Forest Ensemble Model')
...
axs[1, 0].plot(fpr_lstm, tpr_lstm, label='LSTM ROC (AUC = {:.2f})'.format(roc_auc_score(y_test, y_pred_prob_lstm)))
...
axs[1, 1].plot(recall_lstm, precision_lstm, label='LSTM PRC')
...
axs[2, 0].plot(lstm_history.history['loss'], label='Training Loss')
axs[2, 0].plot(lstm_history.history['val_loss'], label='Validation Loss')
axs[2, 0].set_xlabel('Epochs')
axs[2, 0].set_ylabel('Loss')
axs[2, 0].set_title('LSTM Training and Validation Loss')
axs[2, 0].legend(loc='best')

axs[2, 1].plot(lstm_history.history['accuracy'], label='Training Accuracy')
axs[2, 1].plot(lstm_history.history['val_accuracy'], label='Validation Accuracy')
axs[2, 1].set_xlabel('Epochs')
axs[2, 1].set_ylabel('Accuracy')
axs[2, 1].set_title('LSTM Training and Validation Accuracy')
axs[2, 1].legend(loc='best')

# Histogram of Predicted Probabilities
axs[3, 0].hist(y_pred_prob_lstm, bins=50, alpha=0.5, label='LSTM Predicted Probabilities')
